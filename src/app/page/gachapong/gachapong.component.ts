...
export class GachapongComponent implements OnInit { // เรียกใช้งาน class OnInit เพื่อใช้งาน function ngOnit
  ...
  ngOnInit(): void {
    ...
    // ผูกฟังก์ชันกับ this(class นี้) และเก็บไว้ใน property handleLeverClick
  ...
  show_lightBackground_textHint_giftBox () : void {
    ...
    this.h2_text_hint_of_gift!.innerHTML = `You got it!`;
      ...
      onStart: () => {
        ...
        gsap.to(this.img_prize_gift, {
          ...
          onComplete: () => {
            setTimeout(() => {
              // แสดงข้อความ
              gsap.set(this. h2_text_hint_of_gift, { opacity: 1 });
              gsap.from(this.h2_text_hint_of_gift, {
                // y: '-5vh',
                // top: '1vh',
                scale: 0,
                // width: '0vh',
  ...
  //--------------------------------------------------- function แสดงรูปมือและ animation ครั้งที่ 2 หลังจากลูดบอลตกลงมาแล้ว
